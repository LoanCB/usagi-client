...
test("Today view shows only tasks due today", async ({ page }) => {
  ...
  await page.locator(".glass-sidebar").getByRole("button", { name: "Today", exact: true }).click();
...
test("selecting a project shows only its tasks", async ({ page }) => {
  ...
  await page.locator(".glass-sidebar").getByRole("button").filter({ hasText: "Work" }).first().click();
