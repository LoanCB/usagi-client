...
export const DEFAULT_SHORTCUTS: Record<"sortUrgency" | "sortDueDate" | "sortProject", SortShortcut> = {
...
// Returns a display string like "⌘S", "⌥⇧U", or "" if disabled.
export function formatShortcut(s: SortShortcut): string {
  if (!s.key) return "";
  const parts: string[] = [];
