...
export async function backfillSortKeys(db: DbDriver): Promise<void> {
	// One transaction across all three tables: a crash halfway through must not
	...
	await db.transaction(async (tx) => {
		for (const table of ORDERED_TABLES) {
			...
			// oxlint-disable-next-line react-doctor/async-await-in-loop -- intentional: sequential per-table step within the same transaction
