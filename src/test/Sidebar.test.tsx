...
describe("Sidebar — view visibility", () => {
  it("renders Calendar nav item when calendarVisible is true", () => {
    ...
    render(<Sidebar />);
    expect(screen.getByRole("button", { name: /^(calendar|calendrier)$/i })).toBeInTheDocument();
  });

  it("hides Calendar nav item when calendarVisible is false", () => {
    setupStores({ calendarVisible: false });
    render(<Sidebar />);
    expect(screen.queryByRole("button", { name: /^(calendar|calendrier)$/i })).not.toBeInTheDocument();
  });
